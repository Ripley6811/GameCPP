...
bool Texture::GetTextureParams()
{
	...
	if (id > 0)
	{
		...
		glGetTexLevelParameteriv(GL_TEXTURE_2D, mipLevel, GL_TEXTURE_WIDTH, &width); //iv for int not float
		glGetTexLevelParameteriv(GL_TEXTURE_2D, mipLevel, GL_TEXTURE_HEIGHT, &height); //iv for int not float
		return true;
	}
